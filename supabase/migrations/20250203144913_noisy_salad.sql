-- Insert GT Coordinators and School Psychologists
...
VALUES
  ...
  ('Kevin', 'OConnell', 'koconnell@highlineacademy.org'),
  ('Jen', 'Jackson', 'jjackson@kippcolorado.org'),
  ('Leslie', 'Snyder', 'lsnyder@monarchm.com'),
...
SELECT DISTINCT school_id, staff_id, role_id, department_id
FROM gt_assignments
ON CONFLICT (school_id, staff_id, role_id) DO NOTHING;

-- Insert School Psychologist assignments
WITH psych_assignments AS (
  SELECT 
    s.id as school_id,
    st.id as staff_id,
    r.id as role_id,
